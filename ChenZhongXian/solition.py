...
m = 100 # No st ep s in x
c = 1.
dx = 1./m;
beta = 0.8 # beta = c * dt / dx
u = [0] * (m+1)  # I n i t i a l Numeric
u0 = [0] * (m+1)
uf = [0] * (m+1)
dt = beta * dx / c
T_final =0.5
n = int ( T_final /dt ) # N time st ep s
graph1 = gdisplay(width =600, height =500 , xtitle='x', xmin=0, xmax=1,ymin=0 , ymax=1,\
    ytitle='u(x), Cyan=exact, Yellow=Numerical',
title='Advection Eqn: Initial(red),Exact(cyan),Numerical Lax - - Wendroff ( yellow )')
initfn = gcurve( color=color.red)
exactfn = gcurve( color=color.cyan)
numfn = gcurve(color=color.yellow ) # Numerical solution
def plotniExac( ) : # Plot i n i t i a l & exact solution
    for i in range( 0 , m) :
        x = i * dx
        u0[i] = exp(-300.*(x-0.12) **2) # Gaussian i n i t i a l
        initfn.plot(pos = ( 0.01 * i, u0[i])) # I n i t i a l function
